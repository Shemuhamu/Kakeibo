class HouseholdAccount {
    //初期化、データの読み込み
    constructor(userId) {  // ★userId渡すように修正
        this.userId = userId; // 任意のユーザーID
...
// -------------------------------
// Firebase Authentication & HouseholdAccount 初期化
// ---------------------------
let account = null;
...
firebase.auth().onAuthStateChanged(user => {
    if (user) {
        ...
        account = new HouseholdAccount(userId);  // ★ここ重要
...
document.getElementById("setBalanceBtn").addEventListener("click", async () => {
    if (!account) return alert("ログインしてください！");
